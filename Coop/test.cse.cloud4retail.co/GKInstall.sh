...
get_default_versions() {
  ...
  echo "Fetching default versions from Employee Hub Service API..."
  ...
  if [ ! -f "$token_file" ]; then
    echo "Warning: Bearer token file not found at $token_file"
    echo ""
    return
  fi
  ...
  if [ -z "$bearer_token" ]; then
    echo "Warning: Bearer token is empty"
    echo ""
    return
  fi

  # Construct the API URL
  local api_url="https://$base_url/employee-hub-service/services/rest/v1/properties?scope=FP&referenceId=platform"

  # Make the simplified API request using curl (token should be fresh from onboarding)
  ...
  if [ $? -eq 0 ] && [ -n "$response" ]; then
    echo "Successfully fetched default versions from API"
    ...
    # Return the version for the requested component type
    case "$COMPONENT_TYPE" in
      "POS") echo "$pos_version" ;;
      "WDM") echo "$wdm_version" ;;
      "FLOW-SERVICE") echo "$flow_version" ;;
      "LPA-SERVICE") echo "$lpa_version" ;;
      "STOREHUB-SERVICE") echo "$storehub_version" ;;
      *) echo "" ;;
    esac
  else
    echo "Warning: Failed to fetch default versions from API after retries"
    echo ""
  fi
}
