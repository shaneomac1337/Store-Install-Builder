...
if ($UseDefaultVersions) {
    ...
    $tenantId = "001"  # Default tenant ID, could be made configurable
    ...
    # Fetch default versions from API
...
if (-not $offline_mode) {
    ...
    try {
        ...
        if (Get-Command curl.exe -ErrorAction SilentlyContinue) {
            ...
            try {
                curl.exe -L --progress-bar --ssl-no-revoke -o "Launcher.exe" "$download_url"
