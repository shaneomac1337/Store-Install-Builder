...
class TestSystemNameCLIOverride:
    """Test that generated scripts contain --SystemNameOverride CLI parameter override logic"""
    ...
    def test_windows_script_has_system_name_override(self, tmp_path):
        """Test that generated Windows script includes $SystemNameOverride parameter and splatting"""
        from gk_install_builder.generator import ProjectGenerator
        generator = ProjectGenerator()
        self._configure_detection_manager(generator)
        ...
        # Verify $SystemNameOverride parameter in param block
        assert "[string]$SystemNameOverride" in content
        # Verify splatted pass-through
        assert "storeInitArgs['SystemNameOverride']" in content
        assert "Passing SystemNameOverride to store-initialization" in content

    def test_linux_script_has_system_name_override(self, tmp_path):
        """Test that generated Linux script includes --SystemNameOverride CLI parsing"""
        from gk_install_builder.generator import ProjectGenerator
        generator = ProjectGenerator()
        self._configure_detection_manager(generator)
        ...
        try:
            ...
            if output_file.exists():
                ...
                # Verify cli_system_name_override variable
                assert 'cli_system_name_override=""' in content
                # Verify case entry for --SystemNameOverride
                assert "--SystemNameOverride|--systemnameoverride|--systemNameOverride)" in content
                assert 'cli_system_name_override="$2"' in content
                # Verify pass-through to store-initialization
                assert "Passing SystemNameOverride to store-initialization" in content
                # Verify usage string includes --SystemNameOverride
                assert "--SystemNameOverride <name>" in content
        except Exception:
            pass


class TestWorkstationNameCLIOverride:
    """Test that generated scripts contain --WorkstationNameOverride CLI parameter override logic"""
    ...
    def test_windows_script_has_workstation_name_override(self, tmp_path):
        """Test that generated Windows script includes $WorkstationNameOverride parameter and splatting"""
        from gk_install_builder.generator import ProjectGenerator
        generator = ProjectGenerator()
        self._configure_detection_manager(generator)
        ...
        # Verify $WorkstationNameOverride parameter in param block
        assert "[string]$WorkstationNameOverride" in content
        # Verify splatted pass-through
        assert "storeInitArgs['WorkstationNameOverride']" in content
        assert "Passing WorkstationNameOverride to store-initialization" in content

    def test_linux_script_has_workstation_name_override(self, tmp_path):
        """Test that generated Linux script includes --WorkstationNameOverride CLI parsing"""
        from gk_install_builder.generator import ProjectGenerator
        generator = ProjectGenerator()
        self._configure_detection_manager(generator)
        ...
        try:
            ...
            if output_file.exists():
                ...
                # Verify cli_workstation_name_override variable
                assert 'cli_workstation_name_override=""' in content
                # Verify case entry for --WorkstationNameOverride
                assert "--WorkstationNameOverride|--workstationnameoverride|--workstationNameOverride)" in content
                assert 'cli_workstation_name_override="$2"' in content
                # Verify pass-through to store-initialization
                assert "Passing WorkstationNameOverride to store-initialization" in content
                # Verify usage string includes --WorkstationNameOverride
                assert "--WorkstationNameOverride <name>" in content
        except Exception:
            pass
