...
'''
                        ...
                        # Add WorkstationID hardcoding for WDM components in the manual input section
                        # We need to modify the station_detection_code to include WDM hardcoding before manual input
                        wdm_hardcode_before_manual = '''
# Override WorkstationID for WDM components before manual input
if ($ComponentType -eq "WDM") {
    $workstationId = "200"
    $hostnameDetected = $true
    Write-Host "WDM component detected - WorkstationID hardcoded to 200"
    Write-Host "Skipping manual input for WorkstationID"
}
...
"""
                        # Add WorkstationID hardcoding for WDM components before manual input in Bash
                        wdm_hardcode_bash_before_manual = '''
# Override WorkstationID for WDM components before manual input
if [ "$COMPONENT_TYPE" = "WDM" ]; then
  workstationId="200"
  hostnameDetected=true
  echo "WDM component detected - WorkstationID hardcoded to 200"
  echo "Skipping manual input for WorkstationID"
fi
...
'''.format(
    ...
    sh_path=self.detection_manager.detection_config["detection_files"]["STOREHUB-SERVICE"]
            ...
            if wdm_hardcode_check in template:
                print(f"WDM WorkstationID hardcoding already present in {platform} script")
            else:
